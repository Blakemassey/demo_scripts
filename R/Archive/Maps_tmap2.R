...
# Create 2d kernel density maps - isopleth lines, polygons, and rasters
...
rasterize(baea_i, base)
base_crop <- crop(base, extent(baea_i))
plot(base_crop)
xy <- baea_i %>% st_coordinates()
r0 <- rasterize(xy, fun ="count", base_crop)
baea_i_smooth <- smooth_raster_cover(r0)
plot(r0)
get_asp_ratio(baea_i_ext)
