# Done with L-BFGS-S
# Sigma_Ratio_Beta1
df_optimx_fit1 <- readRDS("Results/Bandwidth_Optimization/df_all_opmtix_fit2018-08-17.rds")
df_optimx_fit1 <- readRDS("Results/Bandwidth_Optimization/df_opmtix_fit_LBFGSB_0-50_.5_5-10.rds")
table(df_optimx_fit$sigma)
table(df_optimx_fit$ratio)
table(df_optimx_fit$beta1)

table(df_optimx_fit1$sigma)
table(df_optimx_fit1$ratio)
table(df_optimx_fit1$beta1)
df_optimx_fit1 <- df_optimx_fit1 %>%
  rename(
    bias_est_beta0 = bias_beta0,
    bias_est_beta1 = bias_beta,
    model_logistic = model_ua,
    data_ua = ua_data,
    combo = set) %>%
  dplyr::select(combo, sigma, ratio, beta0, beta1, data_ua, model_logistic,
    est_beta0, est_beta1, aic, bias_est_beta0, bias_est_beta1, model_optimx,
    opt_sigma, opt_beta0, opt_beta1)  %>%
  mutate(bias_opt1_beta0 = beta0 - opt_beta0) %>%
  mutate(bias_opt1_beta1 = beta1 - opt_beta1)
df_optimx_fit1[10,]
df_optimx_fixed_fit <- df_optimx_fit1 %>%
  mutate(fixed_fit = map(data_ua, FitFixedSigma)) %>%
  mutate(fixed_min = map(fixed_fit, ExtractFixedMinAICSigma)) %>%
  mutate(fixed_aic_min = map_dbl(fixed_min,
    function(x) flatten(x)$fixed_aic_min)) %>%
  mutate(fixed_sigma_min = map_dbl(fixed_min,
    function(x) flatten(x)$fixed_sigma_min)) %>%
  mutate(fixed_beta0 = map_dbl(fixed_min,
    function(x) flatten(x)$fixed_beta0)) %>%
  mutate(fixed_beta1 = map_dbl(fixed_min,
    function(x) flatten(x)$fixed_beta1)) %>%
  mutate(bias_fixed_beta0 = beta0 - fixed_beta0) %>%
  mutate(bias_fixed_beta1 = beta0 - fixed_beta1)
saveRDS(df_optimx_fixed_fit, "Results/Bandwidth_Optimization/df_optmix_fixed_fit1.rds")
df_check <- df_optimx_fixed_fit %>%
  dplyr::select(sigma, beta1, opt_beta1, opt_sigma, fixed_sigma_min) %>%
  mutate(opt_beta1 = round(opt_beta1),
    opt_sigma = round(opt_sigma))

# Done with Optimx Defaults
df_optimx_fit2 <- readRDS("Results/Bandwidth_Optimization/df_optmix_5_5-40_0-1500.rds")
table(df_optimx_fit2$sigma)
table(df_optimx_fit2$ratio)
table(df_optimx_fit2$beta1)
df_optimx_fit2 <- df_optimx_fit2 %>%
  mutate(combo = 1:nrow(.)) %>%
  rename(beta0 = intercept,
    data_ua = ua_data,
    model_logistic = model,
    est_beta0 = est_intercept,
    bias_est_beta0 = bias_intercept,
    bias_est_beta1 = bias_beta,
    model_optimx = optimx_results) %>%
  dplyr::select(combo, sigma, ratio, beta0, beta1, data_ua, model_logistic,
    est_beta0, est_beta1, aic, bias_est_beta0, bias_est_beta1, model_optimx)

df_optimx_fit2 <- df_optimx_fit2 %>%
  mutate(opt1_sigma = map_dbl(model_optimx, function(mod) exp(mod[1,3]) +1)) %>%
  mutate(opt1_beta0 = map_dbl(model_optimx, function(mod) mod[1,1])) %>%
  mutate(opt1_beta1 = map_dbl(model_optimx, function(mod) mod[1,2])) %>%
  mutate(opt2_sigma = map_dbl(model_optimx, function(mod) exp(mod[2,3]) +1)) %>%
  mutate(opt2_beta0 = map_dbl(model_optimx, function(mod) mod[2,1])) %>%
  mutate(opt2_beta1 = map_dbl(model_optimx, function(mod) mod[2,2])) %>%
  mutate(bias_opt1_beta1 = beta1 - opt1_beta1) %>%
  mutate(bias_opt1_beta0 = beta0 - opt1_beta0) %>%
  mutate(bias_opt2_beta1 = beta1 - opt2_beta1) %>%
  mutate(bias_opt2_beta0 = beta0 - opt2_beta0)

df_optimx_fit2[1,]
df_optimx_fixed_fit <- df_optimx_fit2 %>%
  mutate(fixed_fit = map(data_ua, FitFixedSigma)) %>%
  mutate(fixed_min = map(fixed_fit, ExtractFixedMinAICSigma)) %>%
  mutate(fixed_aic_min = map_dbl(fixed_min,
    function(x) flatten(x)$fixed_aic_min)) %>%
  ...
  mutate(bias_fixed_beta0 = beta0 - fixed_beta0) %>%
  mutate(bias_fixed_beta1 = beta0 - fixed_beta1)
df_optimx_fixed_fit[1,]
saveRDS(df_optimx_fixed_fit, "Results/Bandwidth_Optimization/df_optmix_fixed_fit2.rds")

df_check <- df_optimx_fixed_fit %>% dplyr::select(sigma, beta1, opt1_beta1, opt1_sigma,
  fixed_sigma_min) %>% mutate(opt1_sigma = round(opt1_sigma))
...
# Plot Intital Parameter Estimates ---------------------------------------------

df_sum <- df_all2 %>% dplyr::select(-c(ua_data, model_ua))
...
df_all_optimx_fit <- df_all_optimx_fit %>%
  mutate(opt1_sigma = map_dbl(model_optimx, function(mod) exp(mod[1,3]) + 1)) %>%
  mutate(opt1_beta0 = map_dbl(model_optimx, function(mod) mod[1,1])) %>%
  mutate(opt1_beta1 = map_dbl(model_optimx, function(mod) mod[1,2])) %>%
  mutate(opt2_sigma = map_dbl(model_optimx, function(mod) exp(mod[2,3]) + 1)) %>%
  mutate(opt2_beta0 = map_dbl(model_optimx, function(mod) mod[2,1])) %>%
  mutate(opt2_beta1 = map_dbl(model_optimx, function(mod) mod[2,2])) %>%
  dplyr::select(-c(ua_data, model_ua, model_optimx))
